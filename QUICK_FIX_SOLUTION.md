...
**修改文件**：
1. ✅ `src/app/api/ai/batch-generate-chapters/route.ts`
   - 修改章节数限制：从100改为50
   - 更新错误提示：添加分批生成建议

2. ✅ `src/app/page.tsx`
   - 添加批量生成章节数检查（handleBatchGenerateChapters函数）
   - 当章节数>50时，弹出警告提示
   - 修改默认章节数：从5改为20
   - 批量生成章节输入框：添加"⚠️ 单次建议≤50章"提示
   - 章节设置输入框：添加"（单次生成建议≤50章）"提示

**修改代码行数**：约15行代码（包括提示文本）

**风险等级**：低（不影响现有功能，仅添加检查和提示）
